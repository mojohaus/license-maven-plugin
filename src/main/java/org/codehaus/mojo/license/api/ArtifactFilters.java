...
public class ArtifactFilters {
    ...
    public static ArtifactFilters of(
    ...
    public ArtifactFilters(
            IncludesExcludes scopeFilters,
            IncludesExcludes typeFilters,
            IncludesExcludes gaFilters,
            boolean includeOptional) {
        super();
    ...
    public boolean isIncluded(Artifact artifact) {
        return scopeFilters.isIncluded(artifact)
                && typeFilters.isIncluded(artifact)
                && gaFilters.isIncluded(artifact)
                && (includeOptional || !artifact.isOptional());
    }
                ...
                if (!line.startsWith("#") && !line.isEmpty()) {
                    ...
                    if (items.length != 3) {
                        throw new IllegalStateException(
                                "Expected 3 space separated tokens on line " + i + " in " + url + " found: " + line);
                    }
                    if ("include".equals(items[0])) {
                        if ("gaPattern".equals(items[1])) {
                            ...
                            throw new IllegalStateException("Expected \"gaPattern\", \"scope\", \"type\" or"
                                    + " \"optional\" after \"" + items[0] + "\" on line " + i + " in " + url
                                    + " found: "
                                    + line);
                        }
                        ...
                        throw new IllegalStateException(
                                "Expected a line starting with \"include\" or \"exclude\" on line " + i + " in " + url
                                        + " found: " + line);
                    }
                }
                i++;
    ...
    static class GaFilter implements ArtifactFilter {
        ...
        public boolean matches(Artifact artifact) {
            return pattern.matcher(artifact.getGroupId() + ":" + artifact.getArtifactId())
                    .matches();
        }
    }
    ...
    static class ScopeFilter implements ArtifactFilter {
    ...
    static class TypeFilter implements ArtifactFilter {
